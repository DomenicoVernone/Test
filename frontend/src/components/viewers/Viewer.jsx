        ...
        {isAnalyzing ? (
          <div className={`absolute inset-0 flex flex-col items-center justify-center gap-4 text-center z-10 backdrop-blur-sm transition-colors ${isDark ? 'bg-slate-900/80' : 'bg-white/80'}`}>
            ...
            <div>
              ...
              <p className={`text-[9px] italic ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Segmentazione volumetrica e proiezione spazio latente...</p>
            </div>
          </div>
        ) : activeTab === '3d' ? (
          (file || niftiUrl) ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-10 z-10 text-center">
              ...
              <p className={`italic text-sm font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>In attesa del segnale anatomico NIfTI...</p>
            </div>
          )
        ) : (
          umapData ? (
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <Activity className={`w-12 h-12 mx-auto mb-4 ${isDark ? 'text-slate-600' : 'text-slate-300'}`} />
              <p className={`italic text-sm font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>I cluster UMAP verranno generati al termine dell'analisi.</p>
            </div>
          )
        )}
      </div>
