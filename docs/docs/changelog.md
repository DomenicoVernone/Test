...
#### 12. `frontend/src/pages/Register.jsx` — Pagina di registrazione pubblica

**Tipo:** Nuovo file  
**Priorità:** P2-Alto

Pagina di registrazione pubblica accessibile su `/register`. Permette ai nuovi utenti di creare un account senza passare da Swagger. Chiama `POST /register` (endpoint pubblico).

**Funzionalità:**
- Campi: username, email (opzionale), password, conferma password
- Validazione password real-time: ✓ Minimo 8 caratteri / ✓ Almeno una maiuscola / ✓ Almeno un numero — i check appaiono solo quando si inizia a digitare e cambiano colore verde/rosso con transizione animata
- Bottone disabilitato finché il form non è valido
- Spinner durante chiamata API
- Errore specifico se username già in uso
- Successo → redirect `/login` con messaggio "Account creato. Accedi ora."
- Link "Hai già un accesso? Accedi" → `/login`
- Stesso identico stile CSS del Login

---

#### 13. `api_gateway/routers/auth.py` — Fix endpoint registrazione pubblica

**Tipo:** Bug fix + Miglioramento  
**Priorità:** P1-Critico

**Problema:** L'endpoint `/signup` esistente richiedeva `require_admin` — qualsiasi chiamata pubblica restituiva 401. La pagina di registrazione frontend non poteva funzionare.
...
**Dopo:** (nuovo endpoint pubblico separato)
```python
@router.post("/register", status_code=201)
def register_user(user: UserCreate,
                  db: Session = Depends(get_db)):
    # Nessun require_admin — ruolo hardcoded a USER
    db_user = User(
...
**Perché:** `/signup` rimane protetto da `require_admin` per uso amministrativo via Swagger. `/register` è il nuovo endpoint pubblico per la self-registration dal frontend. Il ruolo è sempre hardcoded a `USER` lato server — il client non può mai passare `role=admin` (mass assignment protection).

**Impatto:** Senza questo fix la pagina `/register` restituiva sempre 401 rendendo impossibile la registrazione pubblica.
